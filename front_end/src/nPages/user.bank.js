    ...
    methods: {
        ...
        tabChange(index) {
            ...
            axios.post('/user/bindbank',
                ...
                this.alertMsg = "Request timed out!"
            ...
            this.lrzImg(trageFile).then(
                (result) => {
                    if (result.base64Len > imgMaxSize) {
                        this.nowStatus = true;
                        this.errStatus.imgErr = 'File is too large!';
                        return false;
                    } else {
        ...
        async submit() {
            ...
            if (
                (this.postData.index == 1 &&
                    (!this.verify('regStr', 'BankData', 'username', 'Please enter the correct account opening branch') ||
                        !this.verify('regStr', 'BankData', 'name', 'Please enter the correct remark name') ||
                        !this.verify('bank', 'BankData', 'bankcard', 'Please enter the correct bank card number') ||
                        !this.verify('moble', 'BankData', 'moblePhone', 'Please enter the correct SMS verification code!'))
                ) ||
                (this.postData.index == 2 &&
                    (!this.verify('moble', 'postData', 'moblePhone', 'Please enter the correct SMS verification code!') ||
                        !this.verify('wx', 'postData', 'bankcard', 'Please enter the correct WeChat account!'))
                ) ||
                (this.postData.index == 3 &&
                    (!this.verify('moble', 'postData', 'moblePhone', 'Please enter the correct SMS verification code!') ||
                        !this.verify('zfb', 'postData', 'bankcard', 'Please enter the correct Alipay account!')))
            ) { return false; }
            //使用ajax Post方法提交的數據
            let data = this.postData.index != 1 ? this.postData : this.BankData;
            // console.log(data);
            axios.post("/user/bankadd",
                qs.stringify({ data: data }), { rsa: false }
            ).then((res) => {
                if (res.data.status) {
                    //銀行卡只有添加走这里
                    if (this.postData.index == 1) {
                        this.BankData.id = res.data.data.bank_id;
