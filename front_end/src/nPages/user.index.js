    ...
    const user = new Vue({
        ...
        methods: {
            ...
            subFromFun(obj, i) {
                ...
                this.$http.post(url, this.jsFrom).then(({ data, msg, status }) => {
                    this.moveClick = false;
                    if (status) {
                        this.jsTips = 4;
                        this[obj][i].wcg_more -= this.jsFrom.number;
                        console.log(this[obj][i].wcg_more);
                    } else {
                        this.jsTips = msg;
                    }
                }).catch(err => {
            ...
            showHave() {
                ...
                if (Object.is(this.yuBiAccount, null) && this.accountBi == 2) {
                    this.no_data = true;
                }
                if (Object.is(this.userLuck, null) && this.accountBi == 3) {
                    this.no_data = true;
                }
            },
            //打开设置交易密码的弹窗
            ...
            tabAccount(index) {
                ...
                //表头重置
                var url, list;
                this.userInfo && Object.keys(this.coinListbiHave).length === 0 && this.accountBi == 1 ? (url = '/Ajax_User/accountuser', list = 'coinListbiHave') :
                    this.userInfo && Object.keys(this.yuBiAccount).length === 0 && this.accountBi == 2 ? (url = '/Ajax_more/getMore', list = 'yuBiAccount') :
                    this.userInfo && Object.keys(this.userLuck).length === 0 && this.accountBi == 3 ? (url = '/Ajax_more/getMore', list = 'userLuck') :
                    (this.no_data = false, url = '', list = null);
                if (url) {
