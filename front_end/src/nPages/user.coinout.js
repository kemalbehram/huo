...
$(document).ready(function() {
  ...
  const coinoutBox = new Vue({
    data: {
      ...
      //has google or not
      isGoo: false,
      isRgb:document.querySelector('.sel_coin').getAttribute('data-type')
    },
    created() {
      // 获取 加密 公钥
      // this.$getPublicKey();
    ...
    methods: {
      ...
      showRealRMB(money, smallLength) {
        ...
        if (!isNaN(money)) {
          let catchNum='';
          if(/[\%]/.test(coin.rateout)){
            catchNum = money - (money * parseFloat(coin.rateout)/100).toFixed(3);
          }else {
            catchNum= money - parseFloat(coin.rateout);
          }
      ...
      chkkey(type){
        var nVal = this.form.txid;
        var result = true;
        var addReg = /^[0-9a-zA-Z]+$/;
        if (!addReg.test(nVal)) {
          result = false;
        }
        // this.formCompleted.publickey = result;
        return result;
      },
      // input blur
      blur(opt) {
      ...
      submit() {
        ...
        if (!this.fCompleted) {
          ...
          postData.coin=this.form.coin.split('(')[0].toLowerCase();
          // console.log('iiiii',postData);
    ...
    computed: {
      fCompleted() {
        ...
        //       isComplete = true;
        //     } 
        //     else if (key === 'label') {// this.coin.name != 'EOS'&&
        //       isComplete = true;
        //     } 
        //     else {
        //       isComplete = false;
