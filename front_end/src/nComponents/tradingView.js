...
export default (symbolType) => {
    ...
    widget.onChartReady(function() {
        ...
        function changeStaus(tar) {
            let arrTar = tar.parentNode.parentNode.parentNode.childNodes
            for (var i = 0; i < arrTar.length; i++) {
                if (arrTar[i].firstChild) {
                    arrTar[i].firstChild.firstChild.className = "apply-common-tooltip customButton--jqJTfH5-";
                    tar.className = "button sel_btn";
                }
            }
        }
        // 白色
