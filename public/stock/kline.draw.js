...
window.onload = function(){
    !(function(){
        ...
        // var $settings = $('#chart_parameter_settings');

        switch_indic("on");      //开启技术指标
        var b = ChartSettings.get();  //获取设置对象
        var e = b.indics["MA"];     //设置主指标为MA

        b.charts.chartStyle = 'CandleStickHLC';
        var c = ChartManager.getInstance();     //获取ChartManager实例
        // c.getCaptureMouseWheelDirectly = true;   //包裹canvas的div选中状态为true
        c.scale(1.2);                              //让canvas放大
        c.setChartStyle("frame0.k0", 'CandleStickHLC');   //设置主题样式
        ...
        if (!c.setMainIndicator("frame0.k0", 'MA')) {
            c.removeMainIndicator("frame0.k0")
        }
        // c.redraw();
        c.redraw("All", true);
    }());

    $("#chart_show_tools").click();
...
var keyIndex = top.location.href.lastIndexOf('/');
var coinCode = top.location.href.slice(keyIndex+1);
var coinFrom = $(window.parent.document).find("#coin_from").html();
var coinTo = $(window.parent.document).find("#coin_to").html();
coinCode = coinFrom + "_" + coinTo;
...
var GLOBAL_VAR = {
    ...
    button_down: false,
    init: false,
    url: "/ajax_market/kline",
    coin: coinCode,
};
// ?coin=etw_btc&type=5min
...
DarkTheme.prototype.__construct = function() {
    ...
    this._colors[Theme.Color.RangeMark] = "transparent";
    this._colors[Theme.Color.Indicator0] = "#ddd";
    this._colors[Theme.Color.Indicator1] = "transparent";
...
var RequestData = function(showLoading) {
    ...
    $(document).ready(function() {
      GLOBAL_VAR.G_HTTP_REQUEST = $.ajax({
        ...
        beforeSend: function() {
            // 获取当前币种code
            this.data += ("&coin="+ GLOBAL_VAR.coin);
            // 更新请求时间长度
...
function TwoSecondThread() {
    ...
    setTimeout(function(){RequestData()}, 1500);
    // RequestData()
}
function readCookie() {
    ChartSettings.get();
...
* 语言切换
*/
var lang = $(window.parent.document).find("#LANG").html();
if (lang == 'en') {
  lang = 'en-us';
  chart_switch_language("en-us");
}else if (lang == 'cn') {
