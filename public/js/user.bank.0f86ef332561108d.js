...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_nPages_user_bank_js__ = __webpack_require__(842);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_babel_runtime_core_js_promise__ = __webpack_require__(210);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4_nStyle_user_bank_bank_scss__ = __webpack_require__(843);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4_nStyle_user_bank_bank_scss___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_4_nStyle_user_bank_bank_scss__);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8_lrz__ = __webpack_require__(202);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8_lrz___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_8_lrz__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_9__components_tradeLanguagePack__ = __webpack_require__(51);
...
var language = Object(__WEBPACK_IMPORTED_MODULE_9__components_tradeLanguagePack__["a" /* default */])();
console.dir('language', language);

var bank = new __WEBPACK_IMPORTED_MODULE_5_vue__["a" /* default */]({
    data: function data() {
        return {
            realName: '', //真實姓名
            reset: '', //重置二維碼
    ...
    methods: {
        ...
        tabChange: function tabChange(index) {
            ...
            //初始化目前状态
            __WEBPACK_IMPORTED_MODULE_6_axios___default.a.post('/user/bindbank', qs.stringify({
                type: index
            })).then(function (res) {
                _this.realName = res.data.data.user.name;
                if (index == 1) {
                    //銀行卡部分    
                    _this.bankList = res.data.data.bank;
                } else {
                    // 微信 + 支付寶
                    if (res.data.status) {
                        _this.postData = res.data.data.bank[0];
                    } else {
                        _this.postData.bankcard = '';
                        _this.postData.id = '';
                        _this.postData.status = 0;
                        _this.postData.img = '/imgs/zw.png';
                    }
                    _this.imgErr = '√';
                    _this.reset = '';
                }
                _this.postData.index = index;
            }).catch(function (err) {
                _this.alertMsg = "Request timed out!";
                setTimeout(function () {
                    _this.alertMsg = "";
                }, 2000);
            });
        ...
        verify: function verify(reg, pData, itime, errMsg) {
            var _this2 = this;
        ...
        lrzImg: function lrzImg(file) {
            return new __WEBPACK_IMPORTED_MODULE_2_babel_runtime_core_js_promise___default.a(function (resolve) {
                // 压缩 图片
                __WEBPACK_IMPORTED_MODULE_8_lrz___default()(file, { width: 800, quality: 0.6 }).then(function (rst) {
                    resolve(rst);
                }).catch(function (err) {
                    this.errStatus.imgErr = 'Upload timeout!';
                });
            });
        },
        ...
        uploadFile: function uploadFile(e) {
            ...
            var trageFile = e.target.files[0]; //文件
            var size = trageFile.size; //文件大小
            this.lrzImg(trageFile).then(function (result) {
                if (result.base64Len > imgMaxSize) {
                    _this3.nowStatus = true;
                    _this3.errStatus.imgErr = 'File is too large!';
                    return false;
                } else {
                    _this3.errStatus.imgErr = '√';
                    //转换为base64 进行输出
                    _this3.postData.img = result.base64;
                    _this3.reset = true;
                }
            });
        },
                ...
                return __WEBPACK_IMPORTED_MODULE_0_babel_runtime_regenerator___default.a.wrap(function _callee$(_context) {
                    while (1) {
                        switch (_context.prev = _context.next) {
                            case 0:
                                if (!(_this4.postData.index == 1 && (!_this4.verify('regStr', 'BankData', 'username', 'Please enter the correct account opening branch') || !_this4.verify('regStr', 'BankData', 'name', 'Please enter the correct remark name') || !_this4.verify('bank', 'BankData', 'bankcard', 'Please enter the correct bank card number') || !_this4.verify('moble', 'BankData', 'moblePhone', 'Please enter the correct SMS verification code!')) || _this4.postData.index == 2 && (!_this4.verify('moble', 'postData', 'moblePhone', 'Please enter the correct SMS verification code!') || !_this4.verify('wx', 'postData', 'bankcard', 'Please enter the correct WeChat account!')) || _this4.postData.index == 3 && (!_this4.verify('moble', 'postData', 'moblePhone', 'Please enter the correct SMS verification code!') || !_this4.verify('zfb', 'postData', 'bankcard', 'Please enter the correct Alipay account!')))) {
                                    _context.next = 2;
                                    break;
                                }
                            ...
                            case 2:
                                //使用ajax Post方法提交的數據
                                data = _this4.postData.index != 1 ? _this4.postData : _this4.BankData;
                                // console.log(data);

                                __WEBPACK_IMPORTED_MODULE_6_axios___default.a.post("/user/bankadd", qs.stringify({ data: data }), { rsa: false }).then(function (res) {
                                    if (res.data.status) {
                                        //銀行卡只有添加走这里
                                        if (_this4.postData.index == 1) {
                                            _this4.BankData.id = res.data.data.bank_id;
                                            _this4.BankData.status = 1;
                                            _this4.bankList.push(_this4.BankData);
                                            _this4.bankShow = false;
                                        } else {
                                            _this4.postData.moblePhone = '';
                                            _this4.reset = '';
                                            _this4.postData.status = 1;
                                        }
                                    }
        ...
        //获取短信
        getmoble: function getmoble() {
            var _this5 = this;

            //進行手機號和 圖形驗證碼的驗證
            __WEBPACK_IMPORTED_MODULE_6_axios___default.a.post('/user/sendmoble').then(function (res) {
                if (res.data.status) {
                    _this5.numT = 60;
                    _this5.time60();
                } else {
                    bank.errStatus.moblePhone = "Failed to send verification code!";
                    setTimeout(function () {
                        bank.errStatus.moblePhone = "";
                    }, 2000);
                }
            }).catch(function (err) {
                ...
                bank.errStatus.moblePhone = "fail";
                setTimeout(function () {
                    bank.errStatus.moblePhone = "";
                }, 3000);
            });
        ...
        //銀行卡的
        alertFun: function alertFun(close) {
            close == "close" ? this.alertMsg = "" : this.bankShow = !this.bankShow;
        },

        //銀行卡解綁
        openBind: function openBind(id, i, url) {
            var _this6 = this;
            ...
            __WEBPACK_IMPORTED_MODULE_6_axios___default.a.post('/user/' + url, qs.stringify({ id: id, status: status })).then(function (res) {
                if (res.data.status) {
                    if (_this6.postData.index == 1 && url == 'mysave') {
                        _this6.bankList[i].status = parseInt(_this6.bankList[i].status) ? 0 : 1;
                    } else if (url == 'delbank') {
                        _this6.bankList.splice(i, 1);
                    } else {
                        _this6.postData.status = parseInt(_this6.postData.status) ? 0 : 1;
                    }
                } else {
                    _this6.alertMsg = "fail to edit!";
                    setTimeout(function () {
                        _this6.alertMsg = "";
                    }, 2000);
                }
                _this6.alertMsg = res.data.msg;
            }).catch(function (err) {
                setTimeout(function () {
                    _this6.alertMsg = "";
                }, 2000);
...
/* unused harmony default export */ var _unused_webpack_default_export = (bank);
...
//# sourceMappingURL=user.bank.0f86ef332561108d.js.map