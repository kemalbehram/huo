...
class ExchangeBaseModel extends Orm_Base{
	...
	public function post($pKey, $pNum, $coin, &$pUser,$label=''){
		...
		if($pKey==$coinInfo['account'] || $toUid) {
		    if($coinInfo['out_status']==2) Tool_Fnc::ajaxMsg("暫不支持内轉,請更換轉出地址！");
			// 内转
            $toUid = intval($toUid);
			$is_out = 1;
		} else {
        ...
        if($str_len = strpos($coinInfo['rate_out'],'%')){
            ...
            if($pNum<=$charge) return $this->setError($GLOBALS['MSG']['MIN_OUT_NUMBER'].$charge);
        }
        ...
        if($cointype == 'rgb'){
            ...
            $status = $pNum>=$coinInfo['out_limit']?'待审核':'等待';
        }
        ...
        if($label) $exOutData['label'] = $label;
		...
		if(!$tId = $this->insert($exOutData)){
            Tool_Fnc::ajaxMsg('操作失败',0,$this->getLastSql());
