...
class Order_CoinModel extends Orm_Base
{
    ...
    // 更新交易
    public function ajaxcoinOrder($pair2,$coin_pairMo,$dayBeforeCountMo,$space_min5,$space_limit,$i=0)
    {
        ...
        // 订单记录
        $tData = array('max' => 0, 'min' => 0, 'sum' => 0, 'ratio'=>0,'money'=>0);
        $tradesCache = array();
        # 最新50个订单
...
//        $data  = $coin_pairMo->getCoinPrice();
        ...
        $quote  = Cache_Redis::instance('quote')->get($pair2 . '_quote');
        $quote = json_decode($quote,true);
        $prevEndTime = strtotime(date('Ymd', $tOrders[0]['created']));

        //涨跌幅计算
        if($tData['prevEndTime']==$prevEndTime && $quote['openPrice']){
            $openPrice = $quote['openPrice'];
        }else{
            $openPrice = $this->where("opt=1 and coin_from='{$pair['coin_from']}' and coin_to='{$pair['coin_to']}' and created<$prevEndTime")->order('id DESC')->fOne('price');
            if(!$openPrice) $openPrice = $this->where("opt=1 and coin_from='{$pair['coin_from']}' and coin_to='{$pair['coin_to']}'")->order("id asc")->fOne('price');
        }

        $tData['openPrice']=Tool_Math::format($openPrice);

        $tData['ratio'] = 0;
        if($openPrice!=0)
        {
            $tData['ratio'] = round(($tOrders[0]['price']/$openPrice-1)*100, 2);
        }
//        if($pair['coin_from']=='wcg') $tData['ratio'] = -50;
        $tData['prevClose']=Tool_Math::format($openPrice);
        //最新价格
        $tData['price'] = Tool_Str::format($tOrders[0]['price'], $pair['price_float']);

        # 24小时最大、最小
        if($space_min5==$space_limit){
//            echo '$pair_coin_from----'.$pair['coin_from'].'----space_min5:'.$space_min5.'space_limit:'.$space_limit.PHP_EOL;

//            $time = time()-86400;
            # 24小时最大、最小
            if($tMPrice = $dayBeforeCountMo->where("coin='{$pair['coin_from']}'")->fRow()){
                ...
                $tData['money'] = Tool_Math::format($tMPrice['money'], 8)?:0;
            }

            $space_limit +=1;

        }else{
            $max = Tool_Math::format($quote['max'], 8)?:0;
            $min = Tool_Math::format($quote['min'], 8)?:0;
            $tData['max'] = $max>$tData['price']?$max:$tData['price'];
            $tData['min'] = $min>$tData['price']?$tData['price']:$min;
            $tData['sum'] = Tool_Math::format($quote['amount'], 4)?:0;
            $tData['money'] = Tool_Math::format($quote['money'], 8)?:0;
        }
        # 24小时最大、最小
//        if($tMPrice = $model->where("coin='{$pair['coin_from']}'")->fRow()){
//            $tData['max'] = Tool_Math::format($tMPrice['max'], 8)?:0;
//            $tData['min'] = Tool_Math::format($tMPrice['min'], 8)?:0;
//            $tData['sum'] = Tool_Math::format($tMPrice['sum'], 4)?:0;
//            $tData['money'] = Tool_Math::format($tMPrice['money'], 8)?:0;
...
//        $tData['money'] = Tool_Math::format($tData['money'], 8)?:0;
        ...
        $quoteData = array(
            ...
            'openPrice'=>(string)$tData['openPrice'],
            'prevEndTime'=>(string)$prevEndTime
        );
        //法币价
...
//        unset($quoteData['openPrice']);
//        unset($quoteData['prevEndTime']);
        //k线更新最后一个点
        $this->updateKline($pair2, $quoteData);
     ...
     * 写入订单数据
     */
    public function in_order($coin,$trade){
