...
class Trust_CoinModel extends Orm_Base{
	...
	public function cancel($pId, &$pUser, $ctype = 1, $api =false){
		...
		#写入资产明细
        $assetpastlMo = AssetpastModel::getInstance();
        $type = 9;
		$tTrust['status'] = self::STATUS_CANCEL;
        if(!$assetpastlMo->asset_data($pUser,$tTrust,$type,$this->table,$pId)){
            $this->back();
            return $this->setError($GLOBALS['MSG']['SYS_ERROR']);
        }
	...
	public function btc($pData, &$pUser, $api = false){
        ...
        # 写入资产明细
        $AssetpastMo = AssetpastModel::getInstance();
		$type = 9;
		if($pUser['uid']!=33757 && $pUser['uid']!=34558){
		    $opt_type = $pData['type']=='buy'?1:2;
            if(!$AssetpastMo->asset_data($pUser, $pData,$type,$this->table,$tId,$opt_type)){
                $this->back();
                return $this->setError($GLOBALS['MSG']['SYS_ERROR']);
            }
        }
     ...
     * return array()|bool
     */
    public function getListByPrice($price, $pair=array(), $flag='buy', $num=200,$robot_id=0) {
        if(!is_numeric($price) || !is_array($pair)){
            return false;
        }
        if($robot_id){
            $where = array(
                'buy' => "coin_from='{$pair['coin_from']}' and coin_to='{$pair['coin_to']}' and flag='{$flag}' and isnew='N' and price>={$price} and status<2 and uid={$robot_id}",
                'sale' => "coin_from='{$pair['coin_from']}' and coin_to='{$pair['coin_to']}' and flag='{$flag}' and isnew='N' and price<={$price} and status<2 and uid={$robot_id}"
            );
        }else{
            $where = array(
                'buy' => "coin_from='{$pair['coin_from']}' and coin_to='{$pair['coin_to']}' and flag='{$flag}' and isnew='N' and price>={$price} and status<2 and uid!=34558",
                'sale' => "coin_from='{$pair['coin_from']}' and coin_to='{$pair['coin_to']}' and flag='{$flag}' and isnew='N' and price<={$price} and status<2 and uid!=34558"
            );
        }
