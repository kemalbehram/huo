...
class UserModel extends Orm_Base{
	...
	*/
	 ...
	 * @return bool
	 */
	public static function C2cTrustlockUser($key="",$uid = 0,$lock_time=172800){
		if(empty($key) || empty($uid)){
			return false;
		}
		$reids = Cache_Redis::instance();
		$data = array(
			"uid" => $uid,
			"time" => time()+$lock_time
		);
		$result = $reids->set($key,json_encode($data),$lock_time);
		if(!empty($result)){
			return true;
		}
		return false;
    ...
    public function getSpyUsers(){
        ...
        $spyUsers = json_decode($redis->get("SPY_UIDS"),true);
        if(!$spyUsers){
            //redis不存在则写入redis
            $userMo = UserModel::getInstance();
            $spyUsers = $userMo->where("spy=1")->field('uid,mo')->fList();

            if(!$spyUsers) $spyUsers = [];
            $redis->set('SPY_UIDS', json_encode($spyUsers));
        }
        return $spyUsers;
    }
...
>>>>>>> 131b31685869bf761117495eeee195529794a56a
