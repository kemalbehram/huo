...
$orderCoinMo = Order_CoinModel::getInstance();
     ...
     * 获取COIN_PAIR ID
     */
    public function getCoinPairId($market){

        $market = strtolower($market);
        $redis = Cache_Redis::instance();
        $cKey  = 'COIN_PAIR_ID';
        $data = json_decode($redis->get($cKey), true);

        if(!isset($data[$market]))
        {
            $pairs = $this->field('ro_id,name')->where(['status'=>1])->fList();
            foreach ($pairs as $v){
                $data[$v['name']] = $v['ro_id'];
            }
            $redis->set($cKey, json_encode($data));
        }

        if(!$data[$market]) Tool_Fnc::json("获取币种信息失败");
        return $data[$market];
    }
     ...
     * 根据ID获取机器人名称
     */
    public function getCoinPairName($ro_id){
        $redis = Cache_Redis::instance();
        $cKey  = 'COIN_PAIR_ID';
        $data = json_decode($redis->get($cKey), true);

        $res = array_search($ro_id,$data);
        if(!$res)
        {
            $pairs = $this->field('ro_id,name')->where(['status'=>1])->fList();
