	...
	public static function sendCode($user, $aid, $area='+86', $num = 0){
		...
		if($user['mo']){
            if($id = $pc->insert($data)){
                if(MOBILE_CODE){
                    $phone = $area=='+86'?'86'.$user['mo']:$area.$user['mo'];
                    $returnMsg = Tool_SmsMeilian::sendSMS($phone, $data['message']);
...
//                    }
	...
	public static function sendemail($email, $aid)
	{
		...
		if($pc->insert($data)){//插入数据
			$sent = Tool_Fnc::mailto($email, $GLOBALS['MSG']['EMAIL_TITLE'], $data['message']);//火網驗證碼
			if ($sent) {
			return '200';
			} else {//发送失败
				return 0;
	...
	public static function sendregCode($phone, $aid, $area='+86', $num = 0){
		...
		if($phone){
			if($id = $pc->insert($data)){
                if(MOBILE_CODE){
                    $phone = $area=='+86'?'86'.$phone:$area.$phone;//国内或国外
                    $returnMsg = Tool_SmsMeilian::sendSMS($phone,$data['message']);
                    if(strpos($returnMsg,"success")>-1) {
                        return ['code'=>200];
                    }
	...
	public static function selectmessage($type,$aid,$code,$sign='',$phone)
	{
		...
		switch ($type) {
			case 'cn':
				switch ($aid) {
					case 1:
						$mess = "{$sign}验证码{$code}（您好:您尾号{$phone}申请注册或登录，请确认是本人操作），本次验证码5分钟內有效。";
						break;
					case 11:
						$mess = "{$sign}验证码{$code}（您好:您尾号{$phone}申请找回密码，请确认是本人操作），本次验证码5分钟內有效。";
						break;
					case 7:
						$mess = "{$sign}您的验证码是{$code}，如非本人操作，请忽略本短信。";
						break;
					case 8:
						$mess = "{$sign}您的验证码是{$code}，如非本人操作，请忽略本短信。";
						break;
