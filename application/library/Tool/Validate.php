...
class Tool_Validate{
	...
	protected static $typeMsg = [
		...
		'fileExt'     => '上传文件后缀不符',
		'fileMime'    => '上传文件类型不符',
		'chsDashName' => 'chsDashName格式不正确',
		'chsDashComment' => 'chsDashComment格式不正确'
	...
	protected function is($value, $rule, $data = [])
	{
		switch ($rule) {
			...
			case 'chsDashComment':
				// 只允许汉字、字母、数字和下划线_及破折号-
				$result = $this->regex($value, '/^[\x{4e00}-\x{9fa5}a-zA-Z0-9\_\-\,\，\.\。\!\！\"\“\”\(\)\、\?\？\+\-\@\*\s+\￥\$]+$/u');
				break;
			case 'activeUrl':
				// 是否为有效的网址
			...
			case 'chsDashName':
				// 只允许汉字、字母、数字和·
				$result = $this->regex($value, '/^[\x{4e00}-\x{9fa5}a-zA-Z0-9\·\.\s]+$/u');
				break;
			default:
...
/**
   ...
   */
	...
	static function mo($pStr){
		...
		return preg_match("/13[0-9]{9}|15[0-9]{9}|17[0-9]{9}|18[0-9]{9}|147[0-9]{8}/", $pStr);
	}
     ...
     *
     * */
	static function validatemo($pStr,$uid){
		if(!is_numeric($pStr))
		{
			return false;
		}
		$model = new UserModel();
		$count = $model->where("mo=".intval($pStr))->count();
		if($count>=2)
		{
			return false;
		}
...
//		print_r($pStr);die;36242519950205021/
...
//        $id18   = '/^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}/';
	 ...
	 * time: 2017-6-23 lbj
	 */
	static function identifyHG($pStr){
		$id15   = "/^[A-Z][0-9]{6}\([0-9A]\)$/";
		$id18   = "/^[157][0-9]{6}\([0-9]\)$/";
		if(preg_match($id15, $pStr)){
			return TRUE;
	...
	static function name($pStr){
		...
		return preg_match("/^([\x{4e00}-\x{9fa5}]{1}[\x{4e00}-\x{9fa5}·]{0,18}[\x{4e00}-\x{9fa5}]{1}$)|(^[{a-zA-Z}]{1}[{a-zA-Z}\s]{0,18}[{a-zA-Z}]{1}$)/", $pStr);
	}
	 ...
	 * 密码允许字符
	 */
	static function pwd($pStr)
	{
		return preg_match('/^[a-z\d~!@#$^&*()%_+-=|:;,.<>\/?]+$/i', $pStr);
	}

	/**
	 * 验证 护照
	 */
	static function passport($pStr){
	     //return preg_match("/^1[45][0-9]{0}|G[0-9]{8}|P[0-9]{7}|S[0-9]{7,8}|D[0-9]+$/", $pStr);
	     return preg_match("/^(P\d{7}|G\d{8}|S\d{7,8}|D\d+|1[4,5]\d{7})$/", $pStr);
		//		return preg_match("/^1[45][0-9]{7}|([P|p|S|s]\d{7})|([S|s|G|g]\d{8})|([Gg|Tt|Ss|Ll|Qq|Dd|Aa|Ff]\d{8})|([H|h|M|m]\d{8，10})$/", $pStr);
	}
    ...
    public function eos()
	{
		$tool = new Tool_Upload();
		$newfile = $tool->randStr(mt_rand(5,6)) .base_convert(date("ymdHis"),12,36) . $tool->randStr(mt_rand(2,6)) ;
		return $newfile;
	}
