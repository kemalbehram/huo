...
class Tool_Pinyin {
   ...
   * @return string
   */
  static function ascii2py($pAscii) {
    if ($pAscii > 0 && $pAscii < 160) return chr($pAscii);
    if ($pAscii < -20319 || $pAscii > -10247) return '';
    foreach (Tool_Pinyin::$map as $k => $v) if ($v <= $pAscii) break;
    return $k;
  }
