...
class Tool_Math
{
	 ...
	 * 处理%或不带%
	 */
	static function percent($number,$rate){
        if($str_len = strpos($rate,'%')){
            $charge = Tool_Math::mul((floatval(substr($rate,0,$str_len)))/100,$number,8);//*$number;//Tool_Math::mul((substr($rate,0,$str_len))/100,$number,8);
        }else{
            $charge = floatval($rate);
        }
