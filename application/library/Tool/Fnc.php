...
class Tool_Fnc{
	...
	static function mailto($pAddress, $pSubject, $pBody, $pCcAddress = NULL){
		...
		if(!$mail){
			require APPLICATION_PATH.'/application/library/Source/PHPMailer/PHPmailer.php';
//                preg_replace( '/Tool/' ,'' , dirname(__FILE__)) . 'Source/PHPMailer/PHPmailer.php';
			$mail = new PHPMailer();
			$mail->IsSMTP();
			$mail->CharSet = 'utf-8';
			$mail->SMTPAuth = true;
     ...
     * @param string $pStatus ajax返回类型
     */
    static function json($data, $pType = 'json'){
