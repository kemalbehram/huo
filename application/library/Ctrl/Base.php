...
abstract class Ctrl_Base extends Yaf_Controller_Abstract{
	 ...
	 * @return bool
	 */
	protected function _validata_user(){
		$this->_session();
		$userforbidden = new UserForbiddenModel();
		$userforbiddenInfo = $userforbidden->where(['uid'=>$this->mCurUser['uid'],"status" => 0])->fRow();
		if(empty($userforbiddenInfo)){
			return true;
		}
		return false;
	}

	protected function _User_finance(){
		$this->_session();
		$user = new UserModel();
	 ...
	 * ajax请求校验
	 */
	public function ajax_validata_user(){
		$result = $this->_validata_user();
		if($result === false){
			$this->ajax("您已被冻结，请联系管理员！",0);
		}
	}
	 ...
	 * @return bool
	 */
	public function ajaxLockUserTrust($key = ""){
		$result = $this->_c2cLockUserTrust($key);
		if($result !== true){
			$data = json_decode($result,true);
			$time = $data['time']-time();
			$hous = ceil($time/3600);
			$this->ajax("您已被禁止挂单，请在".$hous."小时后重试");
		}
	}

	public function _c2cLockUserTrust($key = ""){
		$redis = Cache_Redis::instance();
		$lockResult = $redis->get($key);
		if($lockResult === false){
			return true;
		}
		return $lockResult;
	...
	public function ajax_user_finance(){
		$result = $this->_User_finance();
		if($result === false){
			$this->ajax("您的资产存在异常。请联系管理员！",0);
		}
	}
     ...
     * AJAX输出
     */
    protected static function ax($data){
	...
	protected function checkReqToken($token='')
	{
		...
		if($token)
		{
			...
			if(!isset($_COOKIE[$key]) || $_COOKIE[$key]!=$token)
			{
				if(!isset($_SERVER["HTTP_X_REQUESTED_WITH"]) || strtolower($_SERVER["HTTP_X_REQUESTED_WITH"])!="xmlhttprequest")
				{
					die('<script>alert("'.$GLOBALS['MSG']['PAGE_EXPIRED'].'(security by www.huocoin.com)")</script>');
				}
				else
				{
