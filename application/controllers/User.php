...
//获取易币用户余额
    ...
    public function bankaddAction(){
        ...
        if($bankdata["data"]["id"]){
            ...
            if($savebank){
                $this->ajax('編輯成功!',1,['bank_id' => $bankdata["data"]["id"]]);
            }else{
                $this->ajax('編輯失敗!',0);
            }
        }else{
            ...
            if($addbank){
                $this->ajax('添加成功!',1,['bank_id' => $addbank]);
            }else{
                $this->ajax('添加失敗!',0);
            }
        }
    ...
    public function sendmobleAction(){
        $uid = $this->mCurUser['uid'];
        $username = 'erlpmqa';  //用户名
        $password_md5 = '02d17c10e09a89a893fb0cf479f47cac';  //32位MD5密码加密，不区分大小写
        $apikey = '7dd0352828d4f14a42526153c70f29e3';  //apikey秘钥（请登录 http://m.5c.com.cn 短信平台-->账号管理-->我的信息 中复制apikey）
