...
class Robot_CountLogicController extends Ctrl_Base{
    ...
    public function income($input){
        ...
        if($type==1){
            $typewherebuy = "buy_uid = '$uid' and buy_uid != sale_uid and buy_uid!=2 && sale_uid!=2 and buy_uid!=17797 && sale_uid!=17797";// and buy_uid!=42255 and sale_uid!=42255
            $typewheresell = "sale_uid = '$uid' and buy_uid != sale_uid and buy_uid!=2 && sale_uid!=2 and buy_uid!=17797 && sale_uid!=17797";// and buy_uid!=42255 and sale_uid!=42255  //何飞
        } else if($type==2){
            $typewherebuy = "buy_uid != '$uid' and sale_uid !=2";
            $typewheresell = "sale_uid != '$uid' and buy_uid !=2";
        }else{
    ...
    public static function trades_level($input,$uid)
    {
        ...
        if ($own) {
            ...
            $ownWhere = "uid != $uid and uid!=2 and uid!=106 and uid!=34558";
        }
        ...
        $mo = new Orm_Base();
        $buy = $mo->query("select numberover,price from trust_{$coin}coin where $ownWhere and (status='0' or status='1') and flag='buy'") ;
        $sell = $mo->query("select numberover,price from trust_{$coin}coin where $ownWhere and (status='0' or status='1') and flag='sale'");
        ...
        for ($i=0;$i<=$limit;$i++){
            ...
            $res['buy_mum'][$i] = 0;
            foreach ($buy as $k => $v) {
                if(($v['price']<=$kline-$size*($i-1) && $v['price'] > $kline-$size*$i && $i) || ($v['price']>$kline && $i==0)){
                    $res['buy_total'][$i] += 1;
...
//        $spyUser = $userMo->where("spy=1")->field('uid')->fList();
        if($input['type']){
            $where = "buy_uid!=sale_uid and (buy_uid=$uid or sale_uid=$uid) and buy_uid!=2 and sale_uid!=2 and buy_uid!=17797 && sale_uid!=17797";// and buy_uid!=42255 and sale_uid!=42255
        }else{
