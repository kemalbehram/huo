...
class Robot_ApiLogicController extends Ctrl_Base
{
    ...
    public function asset($uid){
        ...
        foreach ($coins as $v){
            $exchange[] =  [
                "frozen"=> floatval($user[$v.'_lock']),
                "currency"=> strtoupper($v),
                "fee"=>"0",
                "free"=> floatval($user[$v.'_over'])
            ];
        }
...
//            $this->ajax($trustCoinMo->getError(2));
    ...
    //查询订单
    public function getOrder($input,$market){

        $arr = explode('_',$market);
        $id = substr($input['order_id'],3);

        $Trust_CoinMo = Trust_CoinModel::getInstance();
        $Trust_CoinMo->designTable($arr[0]);
        $trust = $Trust_CoinMo->where("id=$id")->fRow();

        if($trust){

            $side = $trust['flag']=='buy'?'bid':'ask';
            $status = [0=>'open',1=>'open',2=>'closed',3=>'canceled '];

            $res['data'] = [
                'id' => (int)$input['order_id'],
                'remain' => $trust['numberover'],
                'deal_amount' => $trust['numberdeal'],
                'fee_symbol' =>$arr[1],
                'side' => $side,
                'tx' => "0.0",
                'status' => $status[$trust['status']],
                'type' => 'limit',
                'amount' => $trust['number'],
                'client' => 'api',
                'fee' => '0.0',
     ...
     * 查询当前交易对的所有订单
     */
    public function getOrders($uid,$market){

        $arr = explode('_',$market);
        $ro_id = Coin_PairModel::getInstance()->getCoinPairId($market);

        $Trust_CoinMo = Trust_CoinModel::getInstance();
        $Trust_CoinMo->designTable(strtolower($arr[0]));

        $trusts = $Trust_CoinMo->where("uid=$uid and status<2")->order('id desc')->limit(20)->fList();

        if($trusts){
            $status = [0=>'open',1=>'open',2=>'closed',3=>'canceled '];
            foreach ($trusts as $v){
                $side = $v['flag']=='buy'?'bid':'ask';

                $res['data'][] = [
                    'account' => 'exchange',
                    'client' => 'api',
                    'amount' => $v['number'],
                    'deal_amount' => $v['numberdeal'],
                    'fee' =>'0.0',
                    'fee_replace' => '0.0',
                    'fee_symbol'=>$arr[1],
                    'id' => (int)($ro_id.$v['id']),
...
//        $_POST = $input;
//
//        $order = Orm_Base::getInstance()->table("trust_{$input['coin_from']}coin")->where("id={$input['id']}")->fRow();
//
//        if($order){
