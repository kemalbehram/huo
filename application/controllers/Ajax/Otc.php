...
class Ajax_OtcController extends Ajax_BaseController{
    ...
    public function issue_trustAction(){
        ...
        if($type==2 && $userInfo['usdt_over']<1000) $this->ajax("您的资金余额不足1000，无法发起卖单",0,["repeat"=>$newRepeat]);
...
//        }
        ...
        //如果是买入判断用户账户余额是否足够
        if($type==1 && $userInfo['usdt_over']<$buy_total){
            Tool_Lock::del($this);
            $this->ajax("您的余额不足");
        }
...
//            $this->ajax(1,0,[$oup_id,$tup_id,$buy_id]);
            ...
            $lock_usdt = $mo->table("otc_trust_{$coin}")->where("id={$order['trust_id']}")->fOne('lock_usdt');
            //if($lock_usdt<=0) $this->ajax("撤销失败，您的保证金已扣完,请联系管理员", 0, ["repeat" => $newRepeat]);

            $de_lock = $cancel_num>=3 && $lock_usdt>0?100:0;
            $uup_id = $mo->exec("update user set otc_cancel_num=otc_cancel_num+1,usdt_lock=usdt_lock-{$de_lock} where uid=$uid");
            //$this->ajax($uup_id);
            if(!$uup_id){
//                $this->ajax($uup_id.'122');
            ...
            //恢复委托
            $tup_id = $mo->exec("update otc_trust_{$coin} set numberover=numberover-{$order['number']},lock_usdt=lock_usdt-{$de_lock},
numberdeal=numberdeal+{$order['number']},buy_fee_on=buy_fee_on-{$order['buy_fee']},sale_fee_on=sale_fee_on-{$order['sale_fee']},status=0 where id={$order['trust_id']}");
            if($trust['type'] == 2) {
                //更新买家资金
                $total_price = Tool_Math::add(Tool_Math::mul($order['number'], $order['price']), $order['buy_fee']);
...
//            $this->ajax(111111);
            ...
            $lock_usdt = $mo->table("otc_trust_{$coin}")->where("id=$tid")->fOne("lock_usdt");
...
//            }
            ...
            $up_id = $mo->table("otc_trust_{$coin}")->where(['id'=>$tid])->update(['status'=>2,'thaw_usdt'=>$lock_usdt,'updated'=>time(),'updated_date'=>date('Y-m-d H:i:s')]);
            if(!$up_id){
                $mo->back();
                $this->ajax("撤销失败",0,["repeat"=>$newRepeat]);
            }
            $newUserInfo = $oldUserInfo;
            if($trust['type'] == 2 && $lock_usdt) {//&& !$un_trust &&
