...
class Ajax_OtcController extends Ajax_BaseController{
    ...
    public function getTrustWallet($trust_uid = 0,$coin = ""){
        $mo = Orm_Base::getInstance();
        $walletList = $mo->table("otc_address")->field("id,coin,type")->where(['uid' => $trust_uid,"coin" => $coin,'status'=>1])->fList();//->order('id desc')->limit(1)
        return $walletList;
    }
    ...
    public function issue_trustAction(){
        ...
        if($total_trust>=3) $this->ajax("您还有3笔相同类型的订单未完成",0,["repeat"=>$newRepeat]);
        ...
        if($type==2 && $userInfo['cnyx_over']<1000) $this->ajax("您的资金余额不足1000，无法发起卖单",0,["repeat"=>$newRepeat]);
...
//        $this->ajax(1,0,[$total_trust==0,$total_trust,"repeat"=>$newRepeat]);
...
//        }
        ...
        $mo = Orm_Base::getInstance();
        if(!$idcard = $mo->table("autonym")->where("uid=$uid and status=2")->fOne("idcard")){
            Tool_Lock::del($this);
            $this->ajax("您还未通过实名认证，暂无法发起交易",0);
        }
        ...
        if(!$otc_address){
            Tool_Lock::del($this);
            $this->ajax("您还未绑定{$coinS}钱包地址，暂无法发起交易");//,0,["repeat"=>$newRepeat]
        }
        $mo->table = "otc_trust_$coin";
        ...
        if($trust['uid']==$uid){
            Tool_Lock::del($this);
            $this->ajax("不可以交易自己的订单");
        }
        if($trust['numberdeal']<$number){
            Tool_Lock::del($this);
            $this->ajax("该委托剩余数量不足");
        }
        if($trust['min_number']>$number && $trust['numberdeal']>$number){
            Tool_Lock::del($this);
            $this->ajax("最小交易数量为{$trust['min_number']}个");
        }
        if(($type==1 && $trust['type']!=2) || ($type==2 && $trust['type']!=1)){
            Tool_Lock::del($this);
            $this->ajax("交易类型不匹配");
        }
        if($trust['status']!=0){
            Tool_Lock::del($this);
            $this->ajax("该订单已经完成或已撤销，请刷新当前页面！");
        }

        $userInfo = UserModel::getInstance()->fRow($uid);

        $coinConf = CoinModel::getInstance()->where(['name'=>$coin])->field('otc_open,otc_buy_feerate,otc_sale_feerate')->fRow();
        if(!$coinConf['otc_open']){
            Tool_Lock::del($this);
            $this->ajax("该交易已关闭");
        }

        if ($userInfo['pwdtrade'] != $pwdTrade = Tool_Md5::encodePwdTrade($pwdTrade,$userInfo['prand'])){
            Tool_Lock::del($this);
            $this->ajax('交易密码错误');
        }
        ...
        //如果是买入判断用户账户余额是否足够
        if($type==1 && $userInfo['cnyx_over']<$buy_total){
            Tool_Lock::del($this);
            $this->ajax("您的余额不足");
        }
...
//                }
    ...
    public function buy_confirmAction(){
        ...
        $user = new UserModel();
        $asset = new AssetpastModel();
        $oldBuyUserInfo = $user->fRow($order['buy_uid']);
        $oldSellUserInfo = $user->fRow($order['sale_uid']);
        //事务开始
        $mo->begin();
...
//            }
        ...
        //更新卖家资金
        if(isset($tup_id) && $tup_id && $trust['type']==2){
...
//                }
    ...
    public function orderCancelAction(){
        ...
        $user = new UserModel();
        $asset = new AssetpastModel();
        $oldUserInfo = $user->fRow($order['buy_uid']);
        if($order['status']){
            //订单已匹配由买家撤销
            //事务开始
            $mo->begin();
...
//            $this->ajax(1,0,[$oup_id,$tup_id,$buy_id]);
            ...
            $cancel_num = $mo->table("user")->where("uid=$uid")->fOne('otc_cancel_num');
            $lock_cnyx = $mo->table("otc_trust_{$coin}")->where("id={$order['trust_id']}")->fOne('lock_cnyx');
            //if($lock_cnyx<=0) $this->ajax("撤销失败，您的保证金已扣完,请联系管理员", 0, ["repeat" => $newRepeat]);

            $de_lock = $cancel_num>=3 && $lock_cnyx>0?100:0;
            $uup_id = $mo->exec("update user set otc_cancel_num=otc_cancel_num+1,cnyx_lock=cnyx_lock-{$de_lock} where uid=$uid");
            //$this->ajax($uup_id);
            if(!$uup_id){
//                $this->ajax($uup_id.'122');
                $mo->back();
                $this->ajax("撤销失败，请重试", 0, ["repeat" => $newRepeat]);
            }
            ...
            //恢复委托
            $tup_id = $mo->exec("update otc_trust_{$coin} set numberover=numberover-{$order['number']},lock_cnyx=lock_cnyx-{$de_lock},
numberdeal=numberdeal+{$order['number']},buy_fee_on=buy_fee_on-{$order['buy_fee']},sale_fee_on=sale_fee_on-{$order['sale_fee']},status=0 where id={$order['trust_id']}");
            if($trust['type'] == 2) {
                //更新买家资金
                $total_price = Tool_Math::add(Tool_Math::mul($order['number'], $order['price']), $order['buy_fee']);
                ...
                if ($oup_id && $tup_id && $buy_id) {
                    $mo->commit();
                    $cancel_num += 1;
                    $this->ajax("撤销成功,您已撤销{$cancel_num}次匹配中的订单，超过3次后每次撤销扣除100保证金，请谨慎操作", 1, ["repeat" => $newRepeat]);
                } else {
                    $mo->back();
                    $this->ajax("撤销失败，请重试", 0, ["repeat" => $newRepeat]);
                }
...
//        $this->ajax(1,0,[$trust,"repeat"=>$newRepeat]);
...
//            $this->ajax(111111);
            ...
            $lock_cnyx = $mo->table("otc_trust_{$coin}")->where("id=$tid")->fOne("lock_cnyx");
...
//            }
            ...
            $up_id = $mo->table("otc_trust_{$coin}")->where(['id'=>$tid])->update(['status'=>2,'thaw_cnyx'=>$lock_cnyx,'updated'=>time(),'updated_date'=>date('Y-m-d H:i:s')]);
            if(!$up_id){
                $mo->back();
                $this->ajax("撤销失败",0,["repeat"=>$newRepeat]);
            }
            $newUserInfo = $oldUserInfo;
            if($trust['type'] == 2 && $lock_cnyx) {//&& !$un_trust &&
