 ...
 * Time: 14:44
 */
class Ajax_C2cController extends Ajax_BaseController{
    ...
    public function ajaxorderAction(){
        ...
        foreach ($list as $k => $v) {
            $data['list'][] = array(
                ...
                'fee'=>$v['fee'],
                'moble'=>$v['moble'],
                'deal_time'=>empty($v['deal_time'])?0:(strtotime(date("Y-m-d H:i:s",$v['deal_time']))+(30*60)),
                'deal_id'=>$v['deal_id'],
                'selltype'=>$v['selltype'],
                'pay_time'=> empty($v['pay_time'])?0:(strtotime(date("Y-m-d H:i:s",$v['pay_time']))+(30*60)),
                'addtime'=>date('Y-m-d H:i:s', $v['addtime']),
                'status'=>$v['status'],
                'orderprice' => $C2ctradelog->getOrderPrice($v['type'],$v['tradeno'])
            );
            if($v['type']==1){
...
//
//
//        if(!$type || !$price) $this->ajax('请输入金额',0,["repeat"=>$newRepeat]);
//        if ($price < 100) $this->ajax('交易的金额最低为100！',0,["repeat"=>$newRepeat]);
//        if ($price % 100 != 0) $this->ajax('交易价格必须是100的整数倍！',0,["repeat"=>$newRepeat]);
//        if (!is_numeric($price) || strpos($price, ".") !== false) $this->ajax('请检查交易的金额',0,["repeat"=>$newRepeat]);
//
//        $AutonymModel = new AutonymModel();
//        if (!$idcard = $AutonymModel->where("uid={$uid} and status=2")->fRow()) $this->ajax('请先实名认证！',0,["repeat"=>$newRepeat]);
//
//        //获取验证码
//        for (; true;) {
...
//
//        //验证交易密码
//        if ($userInfo['pwdtrade'] != $pwdtrade = Tool_Md5::encodePwdTrade($pwdtradeval,$userInfo['prand'])) $this->ajax('交易密码错误',0,["repeat"=>$newRepeat]);
//
//        //买单用户 未付款点击已付款，只允许每个用户存在两笔这样的订单
//        if($type==1){
//            $buy_count = $c2ctradeMo->where("uid={$uid} and status!=1 and status!=3 and type=1")->fOne("count(id)");
//            if ($buy_count) $this->ajax('您还有未完成交易的订单',0,["repeat"=>$newRepeat]);
//        }
//
//        if($type==2){
//            if($price > 50000){
...
//        if(!$userBank) $this->ajax("请绑定并开启可用的付款方式",0,["repeat"=>$newRepeat]);
...
//        //修改订单状态
//
//        if(!$id) $this->ajax("参数错误",0,["repeat"=>$newRepeat]);
//
//        $trade = $C2ctrade->where(['id' => $id,'type'=>1])->fRow();
//        if(!$trade) $this->ajax('该订单不存在',0,["repeat"=>$newRepeat]);
//        if ($trade['status'] == 1) $this->ajax('该订单已完成!',0,["repeat"=>$newRepeat]);
//        if ($trade['status'] == 3) $this->ajax('该订单已撤销',0,["repeat"=>$newRepeat]);
//        if ($trade['deal_id'] == 0) $this->ajax('该订单正在匹配中',0,["repeat"=>$newRepeat]);
//
//        $mo = Orm_Base::getInstance();
...
//
//        if(!$order) $this->ajax('该订单不存在',0,["repeat"=>$newRepeat]);
//        if ($order['deal_id']==0) $this->ajax('订单还未匹配!',0,["repeat"=>$newRepeat]);
//        if ($order['status'] == 1 || $order['status'] == 3) $this->ajax('订单已处理，不可以重复操作！',0,["repeat"=>$newRepeat]);
//
//        //查找交易中的订单
...
//        if ($_SESSION["token"] == $token) $this->ajax('该订单已完成,不可重复提交！',0,["repeat"=>$newRepeat]);
//
...
//            $this->ajax("请勿重复提交",0);
//        }
//        if(!$id) $this->ajax("该订单号不存在",0,["repeat"=>$newRepeat]);
//
//        $trade = C2ctradeModel::getInstance()->where(['id' => $id])->fRow();
//        $uMo = new UserModel();
//        $asset = new AssetpastModel();
//
//        if ($trade['status'] == 3) $this->ajax('订单已撤销！',0,["repeat"=>$newRepeat]);
//        // if ($trade['deal_id'] != 0) $this->ajax('订单已匹配不可撤销！', 0);
//        if ($trade['status'] == 1) $this->ajax('订单已完成！',0,["repeat"=>$newRepeat]);
//        //买单撤销
//        if ($trade['type'] == 1) {
//            try{
//                //匹配记录
...
//        } else {
//            //卖单撤销
//            if ($trade['deal_id'] != 0) $this->ajax('订单已匹配成功，无法撤单',0,["repeat"=>$newRepeat]);
//
//            try{
//                //匹配记录
