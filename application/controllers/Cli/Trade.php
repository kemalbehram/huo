...
class Cli_TradeController extends Ctrl_Cli
{
    ...
    //限制时间执行一次SQL
    private $space_min5 = 0;
    private $space_limit = 0;
    ...
    {
        $models = array('Order_Coin', 'Trust_Coin', 'User', 'Address', 'FreeTradeUser','DayBeforeCount','Coin_Pair','FeeBonus','Assetpast');//,''
        foreach ($models as $v)
        {
            $m               = $v . 'Model';
...
//            }catch (Exception $e){
//                echo '程序异常--'.json_encode($e).PHP_EOL;
//            }
    ...
    private function quoteInit($coinName)
    {
        ...
        $orderMo->ajaxcoinOrder($coinName,$this->model['Coin_Pair'],$this->model['DayBeforeCount'],$this->space_min5,$this->space_limit);
    }
    ...
    public function coin2coin($coin, $sleep=1, $delayId=0)
    {
        ...
        do
        {
            ...
            if($trust['uid']==34558){
                $list = $trustMo->getListByPrice($trust['price'], $coin, $trust['flag'] == 'buy' ? 'sale' : 'buy', $pageSize,'34558');
            }else{
...
//            echo "-----------------$coinName---------------".PHP_EOL;

            $min5 = date('i',time());
            $this->space_min5  = $min5;//ceil($min5/5);
            if($this->space_limit-$this->space_min5 != 1) $this->space_limit = $this->space_min5  = $min5;

            $space_limit = $orderMo->ajaxcoinOrder($coinName,$this->model['Coin_Pair'],$this->model['DayBeforeCount'],$this->space_min5,$this->space_limit);
            if($space_limit && $space_limit<60) $this->space_limit = $space_limit;
            if($space_limit == 60) $this->space_limit = $this->space_min5;



            echo PHP_EOL;
            echo $coinName.'-----'.$space_limit;
            echo PHP_EOL;

            echo '前面:'."MIN5:".$this->space_min5.'-- LIMIT:'.$this->space_limit.PHP_EOL;
    ...
    public function insertOrder($datas, $coin)
    {
        ...
        foreach ($datas as $v)
        {
            ...
            $ovalues[] = ['price'=>$v['price'],'number'=>$v['min'],'buy_tid'=>$v['buy']['id'],'buy_uid'=>$v['buy']['uid'],'sale_tid'=>$v['sale']['id'],'sale_uid'=>$v['sale']['uid'],'opt'=>$v['opt'],'created'=>$time,'coin_from'=>$coin['coin_from'],'coin_to'=>$coin['coin_to'],'buy_fee'=>$v['buy_fee'],'sale_fee'=>$v['sale_fee']];
        }
...
//        $sql = "INSERT INTO $table (`price`, `number`, `buy_tid`, `buy_uid`, `sale_tid`, `sale_uid`, `opt`, `created`, coin_from, coin_to, buy_fee, sale_fee) VALUES ";
        ...
        if ($orderMo->exec($sql))
        {
            $max_oid = $orderMo->fOne("max(id)");

            //写入资产明细
            $this->assetDetail($ovalues,$max_oid);

            //插入分红手续费
            $this->feeBonus($ovalues,$max_oid);
    ...
    //插入资产明细
    public function assetDetail($values,$max_id){

        $otable = "order_{$values[0]['coin_from']}coin";
        ...
        foreach ($values as $k=>$v){
            $oid = $max_id-$count+$k+1;
            $from = $this->model['User']->where("uid={$v['buy_uid']}")->fRow();
            $to = $this->model['User']->where("uid={$v['sale_uid']}")->fRow();

            //排除机器人账号数据
            if($v['buy_uid']!=33757 && $v['buy_uid']!=34558){
                $opt_type = 1;
                $this->model['Assetpast']->tradeBuy($v,$oid,0,$from,$v['buy_uid'],$v['sale_uid'],$type,$otable,$opt_type);
            }
            if($v['sale_uid']!=33757 && $v['sale_uid']!=34558){
                $opt_type = 2;
                $this->model['Assetpast']->tradeBuy($v,$oid,0,$to,$v['sale_uid'],$v['sale_uid'],$type,$otable,$opt_type);
            }
        }
    }
     ...
     *
     */
    public function feeBonus($values,$max_id){

        $count = count($values);
        foreach ($values as $k=>$v){
            $oid = $max_id-$count+$k+1;
            //买家手续费
            if($v['buy_fee']>0) $this->feeBonusData($v,$oid,'buy');

            //卖家手续费
            if($v['sale_fee']>0) $this->feeBonusData($v,$oid,'sale');

        }
    }

    public function feeBonusData($value,$oid,$type){

        $feeBonusMo = $this->model['FeeBonus'];
        $userMo = $this->model['User'];

        $uid = $type=='buy'?$value['buy_uid']:$value['sale_uid'];
        $opt = $type=='buy'?1:2;
        if($uid==33757) return false;//机器人账号排除
        $from_uid = $userMo->where("uid=$uid")->fOne('from_uid');
        if(!$from_uid || $from_uid=='') return false;

        $ofee = $type=='buy'?$value['buy_fee']:$value['sale_fee'];
        if($type=='buy' && $value['opt']==1){
            $fee = Tool_Math::mul($ofee*0.3,$value['price'],8);
        }else{
        ...
        if($fid = $feeBonusMo->insert($buy_data)){
            ...
            //写入交易分红明细
            $from_user = $userMo->fRow($from_uid);
            $otable = "fee_bonus";
            $this->model['Assetpast']->tradeBuy($value,$fid,0,$from_user,$from_uid,$uid,6,$otable);
        }
    }
