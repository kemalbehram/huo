...
class Cli_TradingrobotController extends Ctrl_Cli{
    ...
    // 刷单时间
    private $time_limit = 40;

    // 保留买卖单委托条数
    private $trustCount = 3;
...
//        $this->checkProcessRunningLock();
    ...
    private function work($coin)
    {
        ...
        while (true)
        {
            ...
            $this->dotrust(strtolower($coin['name']),$pricePoint);
            //处理信号
            pcntl_signal_dispatch();

            sleep(rand($this->time_limit,120));
        }
    }
     ...
     * 挂单
     */
    private function dotrust($coin,$pricePoint){
        ...
        // 否则如果没有成交单，则委托促成交易
            ...
            // 在市场买一价和卖一价之间随机
            $price = $this->getRangePrice($marketPrice['buy_one'], $marketPrice['sale_one'],$pricePoint);
            ...
            $trust_datas = array(
                ...
                2 => array('type'=>'out', 'price'=>round($price*(1+$this->rate), $pricePoint), 'number'=>$trust_number1, 'coin_from' => $arr[0],'coin_to'=>$arr[1], 'trust_type'=>0),
                3 => array('type'=>'in', 'price'=>round($price*(1-$this->rate), $pricePoint), 'number'=>$trust_number2, 'coin_from' => $arr[0],'coin_to'=>$arr[1], 'trust_type'=>0)
            );
...
2 ORDER BY price asc limit 0,".($saleCount-$this->trustCount));
    ...
    private function getMarket($arr){
        ...
        //获取火網平台用户的买一价和卖一价  不包含机器人 需查询数据库
        //首先获取机器人的uid
        $robot = Cache_Redis::instance('token')->keys('*');
        if(!$robot)
        {
    ...
    private function getRangePrice($min, $max,$pricePoint){
        ...
        $price = Tool_Math::format(rand($min*100000000, $max*100000000)/100000000, $pricePoint);
        return $price;
    }
