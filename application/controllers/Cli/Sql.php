...
//            table('weike_user')->SELECT * FROM weike_user WHERE moble LIKE '%+852%'
    ...
    //删除机器人数据
    public function dropRobotAction(){
        ...
        foreach ($coins as $v) {
            if($v['name']=='usdtk') continue;

            $trusts = $this->mo->table("trust_{$v['name']}coin")->where("(uid=33757 or uid=34558) and status>1")->field('id')->fList();

            foreach ($trusts as $key=>$val){
                $this->mo->table("trust_{$v['name']}coin")->exec("DELETE FROM trust_{$v['name']}coin WHERE id={$val['id']}");
                if($key%1000==0) echo $v['name'].$key.PHP_EOL;
            }
        }
