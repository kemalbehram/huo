...
//            }
            ...
            $assetMo = AssetpastModel::getInstance();

            foreach ($listtransactions as $trans) {
                //无blockhash  不处理
                if (!$trans['blockhash']) continue;
                //小于最小转入额不处理
                if($trans['amount']<$coinconf['min_in']) continue;
                ...
                if ($trans['category'] == 'receive') {
                    ...
                    if (isset($change) && $change && $change['status'] == '成功') continue;

                    //如果小于确认数
                    if ($trans['confirmations'] < $coinconf['confirm']) {
                        if (!$change){
                            $exchangeModel->insert(['uid'=>$uid,'wallet' =>$trans['address'],'txid' => $trans['txid'],'confirm'=>$trans['confirmations'],'number' =>$trans['amount'], 'number_real' => $trans['amount'],'created'=>time(),'opt_type'=>'in','status'=>'确认中']);
                        }else {
                            if ($coinconf['confirm'] > $change['confirm']) {
                                $ex_up_id = $exchangeModel->where(['id' => $change['id']])->update(['confirm' => $trans['confirmations'], 'updated' => time()]);
                            }
                        }
                        continue;
                    }
                    ...
                    $mo->begin();
                    try{
                        if ($change && $change['status']=='确认中') {
                            $in_id = $exchangeModel->where(['id'=>$change['id']])->update(['updated' => time(), 'opt_type' => 'in','status'=>'成功','confirm'=>$trans['confirmations']]);
                        } else {
                            $in_id = $exchangeModel->insert(['uid'=>$uid,'wallet'=>$trans['address'],'txid'=>$trans['txid'],'number'=>$trans['amount'],'number_real'=>$trans['amount'], 'created' => time(), 'opt_type' => 'in','status'=>'成功','confirm'=>$trans['confirmations']]);
                        }
                        $save_id = $mo->exec("update user set {$coin}_over = {$coin}_over+{$trans['amount']} where uid={$uid}");
                        if($in_id && $save_id) $mo->commit();

                        $userinfo = $userModel->where("uid=$uid")->fRow();
                        $oid = isset($change['id'])?$change['id']:$in_id;
                        $otable = "exchange_{$coin}";
                        $assetMo->wallet_asset($userinfo,8,$coin,$oid,$otable,0,1);
    ...
    public function ethAction($coin){
        ...
        try{
            ...
            $rpcurl = Yaf_Registry::get("config")->api->rpcurl->$coin;
            if (empty($rpcurl)) Tool_Del::lock($lockfile,'钱包地址不存在');
            $coinconf['password'] = Tool_Wallet::encrypt($coinconf['password'],'D',Tool_Wallet::key());
...
//                        if($coin!=$addr['coin']) continue;
                        ...
                        $coinInfo = $coinModel->where(['status' => 0, 'name' => $addr['coin']])->fRow();
                        //小于最小转入额不处理
                        if($true_amount<$coinInfo['min_in']) continue;
                        ...
                        if ($final_amount > 0.005) {
                            ...
                            try {
                                ...
                                if($ad_id && $ex_id) {
                                    ...
                                    $userinfo = $userModel->where("uid={$addr['uid']}")->fRow();
                                    $otable = "exchange_{$addr['coin']}";
                                    $assetMo->wallet_asset($userinfo,8,$coin,$ex_id,$otable,0,1);
                                    //转入成功后，执行归集到指定地址
                                    $tradeInfo_token = [[
                                        'from' => $trans->to,
                                        'to' => $coinInfo['account'],
                                        'gas' => '0x15f90',//0x4a380
                                        'value' => $true_amount - 0.005,
                                        'gasPrice' => '0x4a817c800' //$CoinClient->eth_gasPrice()
                                    ]];
                                    $CoinClient->eth_sendTransaction($addr['address'], $addr['secret'], $tradeInfo_token);
                                }else {
                                    $mo->back();
                                }
                            }catch (Exception $e) {
...
//                die;
                    ...
                    if ($coinconf_token = $coinModel->where(['status' => 0, 'token_address' => $trans->to])->fRow()) {
                        ...
                        //实列化一个对应币种的类
//                        if($coin!=$coinconf_token['name']) continue;
...
//                    Tool_Out::p($exModel);
                        ...
                        //判断该交易是否轮询
                        if ($exModel->where(['txid'=>$trans->hash])->fOne('id')) continue;

//                    Tool_Out::p($to);die;

                        if ($user_token = $addressModel->where(['address' => $to])->fRow()) {
//                            if($coinconf_token['name']=='moa') $coinconf_token['number_float'] = 18;
                            $true_amount_token = floatval($CoinClient->real_banlance_token($CoinClient->decode_hex($token_value), $coinconf_token['number_float']));

                            $coinInfo = $coinModel->where(['status' => 0, 'name' =>$user_token['coin']])->fRow();
                            //小于最小转入额不处理
                            if($true_amount_token<$coinInfo['min_in']) continue;

                            //                            if($coinconf_token['name']=='moa') echo "MOA:".$true_amount_token;
                            ...
                            try{
                                ...
                                if ($u_id && $in_id) {
                                    ...
                                    if ($yue_token < floatval('0.005')) {
                                       ...
                                       $CoinClient->eth_sendTransaction($coinconf['account'], $coinconf['password'], $tradeInfo_token);
                                    }
                                    //代币归集
                                    $value = $CoinClient->encode_dec($CoinClient->to_real_value_token(floatval($true_amount_token) , $coinconf_token['number_float']));
                                    $tradeInfo = [[
                                        'from' => $to,
                                        'to' => $coinconf_token['token_address'],
                                        'data' =>  '0xa9059cbb'. $CoinClient->data_pj($coinconf_token['account'], $value),
                                    ]];
                                     $CoinClient->eth_sendTransaction($user_token['address'], $user_token['secret'], $tradeInfo);

                                    $userinfo = $userModel->where("uid={$user_token['uid']}")->fRow();
                                    $otable = "exchange_{$coinconf_token['name']}";
                                    $assetMo->wallet_asset($userinfo,8,$coin,$in_id,$otable,0,1);
                                } else {
                                    $mo->back();
                                }
                            }catch (Exception $e){
...
//        Tool_Out::p($block_info);die;
            ...
            foreach ($block_info as $k=>$v) {
                ...
                //最小额
                if($final_amount<$coinConf['min_in']) continue;
                ...
                foreach ($v['outputs'] as $key => $val) {
                    ...
                    if (preg_match('/([1-9]|[a-z]|[A-Z])/', $v['block_hash']) && $v['block_height'] && $v['block_index']) {
                        ...
                        if ($ex = $exchangeModel->where(['wallet' => $val['address'], 'txid' => $v['tx_id']])->fRow()) {
                            ...
                            //最小额
                            if($amount<$coinConf['min_in']) continue;
                            ...
                            try {
                                ...
                                if ($ex_id && $in_id) {
                                    ...
                                    $userinfo = $userModel->where("uid={$ex['uid']}")->fRow();
                                    $otable = "exchange_{$coin}";
                                    $assetMo->wallet_asset($userinfo,8,$coin,$ex['id'],$otable,0,1);
                                } else {
                                    $mo->back();
                                }
                            } catch (Exception $e) {
...
//            Tool_Out::p($lists);//die;
    ...
    //VSYS系列
    public function vsysAction($coin){
        $coin = $coin ? $coin : 'vsys';
        ...
        if (!$coinconf || $coinconf['type'] != 'vsys') Tool_Del::lock($lockfile,'币种错误');
        ...
        $vsysClient = new Api_Rpc_VsysClient($address, $port, $coinconf['password']);

        $blocks = $vsysClient->blocks($coinconf['block_num']);
        if(!$blocks['version']) Tool_Del::lock($lockfile,'钱包获取数据失败');

        $lists = $blocks['transactions'];
        if(!count($lists)) Tool_Del::lock($lockfile,'该高度无数据');
        if(!$block = $coinModel->exec("update coin set block_num=block_num+1 where name='{$coin}'")) Tool_Del::lock($lockfile,'高度更新失败');

        $exchange = 'Exchange_' . ucfirst($coin) . 'Model';
        $exchangeModel = $exchange::getInstance();

        $addressMo = AddressModel::getInstance();
        ...
        foreach ($lists as $v) {
            ...
            //TYPE不等于2或状态不等于Success跳走
            if($v['type']!=2 || $v['status']!='Success') continue;
            //TXID存在 则跳走
            if ($txid = $exchangeModel->where(['txid' => $v['id']])->fOne('id')) continue;

            //接收地址不存在则跳走
            if(!$uid = $addressMo->where(['address'=>$v['recipient']])->fOne('uid')) continue;

            $amount = Tool_Math::div($v['amount'],100000000,8);
            if($amount<$coinconf['min_in']) continue;
            ...
            $ex_id = $exchangeModel->insert([
                ...
                'created' => time(),
                'bak' => $v['attachment'],
                'mining_fee' => Tool_Math::div($v['fee'],100000000,8),
                'opt_type' => 'in',
                'status' => '成功',
            ]);
            if(!$ex_id){
...
//                                'nonce' => '0x5fd'
                            ...
                            //echo '------gasPrice:'.$tradeInfo[0]['gasPrice'];
...
//                    } while ($sendrs->error != '');//$sendrs->error->code != (-32000)
    ...
    //VSYS归集
    public function vstsMergeAction($coin){
        $mo = Orm_Base::getInstance();
        $coin = $coin?$coin:'vsys';
        $coinconf = $mo->table("coin")->where(['name' => $coin, 'status' => 0])->fRow();

        if(!$coinconf || $coinconf['type']!='vsys') die('币种类型错误');
        ...
        $addressMo = AddressModel::getInstance();
        //筛选数据库中钱包中大于 0.01 的用户 分组
        $address = $addressMo->field('address')->where("coin='{$coin}' and address!=''")->fList();//->limit($i * $offset, 1000)
        ...
        if(count($address) > 0) {
            foreach ($address as $k => $v) {
                ...
                //查询该地址余额是否足够 归集最小数量
                $balance = $vsysClient->get_balance($v['address']);
                if($balance<$coinconf['merge_limit']) continue;

                $number = $balance-$fee;
                $payment = $vsysClient->payment($number,$fee,$v['address'],$coinconf['account']);

                if(isset($payment['error']) || $payment['type']!=2 || !$payment['id']) continue;
        ...
        $balance = $vsysClient->get_balance($coinconf['account']);
        echo $coin . ' 账户总数量：' . $balance . PHP_EOL;
        echo "本次归集总数量:{$mergeNum}".PHP_EOL;
        die('成功');
    }
