...
class Cli_WalletOutController extends Ctrl_Cli{
    ...
    public function  bitAction($coin){
        ...
        try{
            ...
            if(!$exchanges)  Tool_Del::lock($lockfile,'');
...
//            if(in_array($coin,['ltc','btc','doge'])){
                $wallet_pass = Tool_Wallet::encrypt($coinConf['wallet_pass'],'D',Tool_Wallet::key());
//            }else{
//                $wallet_pass = Tool_Wallet::encrypt($coinConf['wallet_pass'],'D',Tool_Wallet::key());
//                $wallet_pass = md5($wallet_pass);//
//            }

            //echo $coinConf['password'].'-------'.Tool_Wallet::encrypt($coinConf['wallet_pass'],'D',Tool_Wallet::key());die;
            //die;

            //传入钱包密码
            $CoinClient->walletpassphrase($wallet_pass, 60);

            //echo $coinConf['password'].'-------'.$wallet_pass;die;
            ...
            foreach ($exchanges as $k=>$v) {
                ...
                if($v['is_out'] || $v['to_uid']){
                    ...
                    try{
                        ...
                        $ex_id = $exchangCoinModel->where(['uid'=>$v['uid'],'id'=>$v['id']])->update(['status'=>'成功','txid' => md5($v['wallet'] . $v['address'] . time())]);
...
//                    if($coin=='bcx') Tool_Out::p($sendrs_arr);
...
//                            if ($v['platform_fee'] > 0) $fee_id = $mo->exec("update user set {$coin}_over={$coin}_over+{$v['platform_fee']} where uid=2");
                            ...
                            if($out_id && $s_user){
                                ...
                                $userinfo = $userModel->where("uid={$v['uid']}")->fRow();
                                $otable = "exchange_{$coin}";
                                $assetMo->wallet_asset($userinfo,8,$coin,$v['id'],$otable,0,2);
                            }else{
                                $mo->back();
                            }
                        }catch (Exception $e){
            ...
            foreach ($exchanges as $k=>$v) {
                ...
                if ($v['is_out'] || $v['to_uid']) {
                    ...
                    try{
                        ...
                        $to_id = $userModel->exec("update user set {$coin}_over={$coin}_over+{$v['number_real']} where uid={$v['to_uid']}");
                        $ex_id = $exchangCoinModel->where(['uid'=>$v['uid'],'id'=>$v['id']])->update(['status'=>'成功','txid' => $txid]);
                        $sa_id = $userModel->exec("update user set {$coin}_lock={$coin}_lock-{$v['number']} where uid={$v['uid']}");
                        if($in_id && $sa_id && $ex_id && $to_id){
                            $mo->commit();
                        }else{
...
//                    Tool_Out::p($sendrs);
                    ...
                    if(($sendrs && !isset($sendrs_arr['status'])) && isset($sendrs->result) && $sendrs->result){
                        echo '执行OK';
                        //手续费
                        $mo->begin();
                        try{
...
//        Tool_Out::p($exchanges);die;
            ...
            //实列化另一个端口类
            $EosClient_Sign = new Api_Rpc_EosClient('172.66.66.187','9999');
            ...
            foreach ($exchanges as $k=>$v) {
                //判断可用余额
                if ($v['number_real'] > $plat_balance){
                    echo '转出数:'.$v['number_real'].';钱包余额'.$plat_balance;
                    continue;
                }
...
//                        $thaw_id = $exchangCoinModel->update(['id'=>$v['id'],'status'=>'等待']);
                    ...
                    $sendrss = $EosClient->push_transaction('huo', $coinConf['password'],$push_transaction_info);

                    echo '1111111111111'.PHP_EOL;
                    echo "<pre>";
                    var_dump($sendrss);
                    $sendrs = $sendrss->transaction_id;
                    if($sendrs){
...
//                    $thaw_id = $exchangCoinModel->update(['id'=>$v['id'],'status'=>'等待']);
    ...
    public function xrpAction($coin){
        ...
        try{
            $CoinModel = CoinModel::getInstance();
            $coinConf = $CoinModel->where(['name' => $coin])->fRow();
            $coinConf['out_password'] = Tool_Wallet::encrypt($coinConf['out_password'],'D',Tool_Wallet::key());
...
//        Tool_Out::p($exchanges);die;
            ...
            //实例化钱包
            $xrpClient = new Api_Rpc_XrpClient($address,$port,$coinConf['out_account'],$coinConf['out_password'],$coinConf['token_address']);
...
//                    $thaw_id = $exchangModel->update(['id'=>$v['id'],'status'=>'等待']);
    ...
    public function vsysAction($coin){
        ...
        if ($coinConf['type'] != 'vsys') Tool_Del::lock($lockfile,'币种错误');
...
//        Tool_Out::p($exchanges);die;
        ...
        foreach ($exchanges as $k => $v) {
            ...
            //找出待转出钱包地址
//            $sender_addr = $addressMo->where(['coin'=>$coin,'uid'=>$v['uid'],'status'=>0])->fOne("address");
            //查询该地址余额是否足够
            $balance = $vsysClient->get_balance($coinConf['account']);

            if($balance<($v['number_real']+$fee)) continue;

//            if($v['platform_fee']<=$fee) continue;//手续费不够 0.1 矿工费
//            $fee_number = $v['platform_fee'] - $fee;
//            $fee_pay = $vsysClient->payment($fee_number,$fee,$coinConf['account'],$coinConf['fee_address']);//手续费转出地址

//            $number_real = $v['number_real']-$fee*2;//扣除两倍矿工费
            $payment = $vsysClient->payment($v['number_real'],$fee,$coinConf['account'],$v['wallet']);

//            Tool_Out::p($payment);die;

            if(isset($payment['error']) || $payment['type']!=2 || !$payment['id']){
                echo $v['id'].'钱包服务器转出币失败';
                continue;
            }
            ...
            $mo->begin();
            try {
                $out_id = $exchangModel->where(['id' => $v['id']])->update(['status' => '成功', 'txid' => $payment['id']]);
                $s_user = $mo->exec("update user set {$coin}_lock={$coin}_lock-{$v['number']} where uid={$v['uid']}");
                ...
                if ($out_id && $s_user) {
                    ...
                    $mo->back();
                    Tool_Del::lock($lockfile,'数据库更新失败');
                }
            } catch (Exception $e) {
                $mo->back();
                Tool_Del::lock($lockfile,'异常');
