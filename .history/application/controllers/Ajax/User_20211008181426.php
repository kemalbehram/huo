           ...
           // $tData = $tMO->fRow('SELECT * FROM email_activate WHERE email = \'' . $email . '\' limit 1');
            ...
            if (!empty($oldactive)) {//更新1
                if ($eMO->where(array('email' => $email))->update($eData)) {
                    ...
                    $res = UserModel::saveEmailRedis(array(
                        'key' => $pKey,
                        'email' => $email,
                    ));
...
//            $userdd = $usermo->where(array('mo' => $data['phone']))->fOne('area');
...
//                }
    ...
    public function coinOutAction()
    {
        ...
        if((($cointype == 'rgb' && $coin!='unih')  || $cointype== 'xrp' || $cointype=='eos') && !$label) $this->ajax('請輸入公鑰', 0, 'label');
...
//            }
    ...
    public function msgAction()
    {
        if($_POST['regtype']=='phone')
        {
            ...
            {
                $user['mo']=$phone;
                $result = PhoneCodeModel::sendCode($user, $action, $area);

                if ($result['code'] == '200' || $result==200)
                {
                    $this->ajax($GLOBALS['MSG']['SMS_SUCCESS'], 1,$result['msg']?$result['msg']:''); //发送成功
                }
                else
                {
...
//充币
    ...
    public function getCoinAddressAction()
    {
        ...
        //VSYS系列
        if($cointype == 'vsys') $wallet = $addressMo->getAddrVsys($this->mCurUser['uid'], $name);
...
//        }
    ...
    public function erifypPhoneAction()
    {
        ...
        if($postdata['regtype']=='email')
        {
            ...
            $this->ajax('邮箱找回登录密码已关闭，请绑定手机用手机验证码找回');
...
//            $this->ajax($data['getpwd_phone'].'--'.$data['area'], '0', 'phone');//手机号不存在
...
//        $this->ajax($this->_request->action);
    ...
    public function rewardAction(){
        ...
        $list = $activeRewardMo->field("id,type,coin,price,status")->where("uid = {$user['uid']} and type = 1 ")-> fList();
        $this->ajax("",1,$list);
    }

    //领取wcg活动奖励
    public function receiveRewardAction(){
        $this->_ajax_islogin();
        $user = $this->mCurUser;
        $id = intval(coll("post","id"));
        if(empty($id)){
            $this->ajax("参数错误",0);
        }
        $activeRewardMo = new ActiverewardModel();
        $reward_info = $activeRewardMo->where("uid = {$user['uid']} and type = 1 and id = {$id} ")->fRow();
        if($reward_info['status'] != 1){
            $this->ajax("该奖励已经领取过了。",0);
        }
        $price = floatval($reward_info['price']);
        $coin = $reward_info['coin'];
        $pUser = UserModel::getInstance()->lock()->fRow($this->mCurUser['uid']);
        $activeRewardMo->begin();
        $up_id = UserModel::getInstance()->exec("update user set {$coin}_over={$coin}_over+{$price} where uid={$this->mCurUser['uid']}");
        if(!$up_id){
            $activeRewardMo->back();
            $this->ajax("领取失败",0);
        }
        ...
        $newUserInfo = $pUser;
        $newUserInfo[$coin.'_over'] = $newUserInfo[$coin.'_over']+$price;
        $asset_result = $asset->InsertAssetData($pUser,$newUserInfo,$id,AssetpastModel::ASSET_TYPE_ACTIVE,$coin,$coin,2);
        ...
        $up_info = $activeRewardMo->where("id = {$id}")->update($up_data);
        if($up_info){
    ...
    public function likesAction(){
        ...
        $list = $activeRewardMo->field("id,type,coin,price,status")->where("uid = {$user['uid']} and type = 2 ")-> fList();
        $this->ajax("",1,$list);
    }

    //领取wcg活动奖励
    public function likesRewardAction(){
        $this->_ajax_islogin();
        $user = $this->mCurUser;
        $id = intval(coll("post","id"));
        if(empty($id)){
        ...
        $activeRewardMo = new ActiverewardModel();
        $reward_info = $activeRewardMo->where("uid = {$user['uid']} and type = 2 and id = {$id} ")->fRow();
...
//        if(isset($lastbonus) && $lastbonus['created']>=$day_time) $this->ajax("您已經領取過了");
        ...
        $user = new UserModel();
        $asset = new AssetpastModel();
        $oldUserInfo = $user->fRow($this->mCurUser['uid']);
        $mo->begin();
        $in_data = [
            'uid'=>$this->mCurUser['uid'],
            'coin'=>$coin,
...
//        $this->ajax(1,0,$data);
        ...
        //交易手续费
        $tradeFees = $mo->table("fee_bonus")->field("sum(fee) total_fee,origin_uid")->where("uid=$uid and type = 1 ")->group("origin_uid")->fList();
        $tradeFees = array_column($tradeFees,'total_fee','origin_uid');

        //平台充值手续费
        $platformFees = $mo->table("fee_bonus")->field("sum(fee) total_fee,origin_uid")->where("uid=$uid and type=2")->group("origin_uid")->fList();
        $platformFees = array_column($platformFees,'total_fee','origin_uid');

        //C2C充值手续费
        $c2cFees = $mo->table("fee_bonus")->field("sum(fee) total_fee,origin_uid")->where("uid=$uid and type=3 ")->group("origin_uid")->fList();
    ...
    */
    public function inviteBonusDetailAction(){
        ...
        $uid = $this->mCurUser['uid'];
        $origin_uid = coll('get','origin_uid');
        $type = coll('get','type');
        ...
        if($type==1){
            $total = $mo->table("fee_bonus")->where("uid=$uid and origin_uid={$origin_uid} and type=1")->order("created desc")->fOne("count(id)");
            $data['list'] = $mo->table("fee_bonus")->field("FROM_UNIXTIME(created) created,price,number,opt,coin_from,oid,fee")->where("uid=$uid and origin_uid={$origin_uid} and type=1")->page($page,$size)->order("created desc")->fList();
//            if($page!=1) $this->ajax($page.'--'.$mo->getLastSql(),1,json_encode($data['list']));
        }elseif($type==2){
            $total = $mo->table("fee_bonu")->where("uid=$uid and origin_uid={$origin_uid} and type=2")->order("created desc")->fOne("count(id)");
            $data['list'] = $mo->table("fee_bonus")->field("FROM_UNIXTIME(created) created,number,fee")->where("uid=$uid and origin_uid={$origin_uid} and type=2")->page($page,$size)->order("created desc")->fList();
        }elseif($type==3){
            $total = $mo->table("fee_bonus")->where("uid=$uid and origin_uid={$origin_uid} and type=3")->order("created desc")->fOne("count(id)");
            $data['list'] = $mo->table("fee_bonus")->field("FROM_UNIXTIME(created) created,number,fee")->where("uid=$uid and origin_uid={$origin_uid} and type=3")->page($page,$size)->order("created desc")->fList();
        }else{
            $this->ajax("参数错误");
        }
